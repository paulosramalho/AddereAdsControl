...
const STATUS_CLS = {
  ...
  SCHEDULED:  "bg-blue-700/50 text-blue-200",
  PUBLISHING: "bg-amber-700/50 text-amber-200",
  PUBLISHED:  "bg-emerald-700/50 text-emerald-200",
...
export default function SchedulePostModal({ open, post, defaultDate, clientId, onClose, onSave, initialValues }) {
  ...
  async function handleRetry() {
    try {
      const res = await api.put(`/clients/${clientId}/scheduled-posts/${post.id}`, { status: "SCHEDULED" });
      if (res.ok) {
        const d = await res.json();
        addToast("Post reagendado para publicação", "success");
        onSave?.(d.post);
        onClose();
      } else {
        const d = await res.json();
        addToast(d.message ?? "Erro ao reagendar", "error");
      }
    } catch {
        ...
        <div className="flex items-start justify-between px-6 py-4 border-b border-slate-700 flex-shrink-0">
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {post?.status && (
                <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${STATUS_CLS[post.status] ?? ""}`}>
        ...
        >
          ...
          {post?.status === "FAILED" && post?.errorMessage && (
            <div className="bg-red-900/30 border border-red-700/50 rounded-lg px-3 py-2.5">
              <p className="text-xs text-red-400 font-medium mb-0.5">Erro ao publicar</p>
              <p className="text-xs text-red-300 leading-relaxed">{post.errorMessage}</p>
            </div>
          )}

          {/* Performance do post publicado */}
          {post?.status === "PUBLISHED" && post?.igMetrics && (
            <div className="bg-emerald-900/20 border border-emerald-700/40 rounded-xl px-4 py-3">
              <p className="text-xs text-emerald-400 font-medium mb-2">Performance no Instagram</p>
              <div className="grid grid-cols-4 gap-2 text-center">
                <div>
                  <p className="text-white font-bold">{post.igMetrics.likes.toLocaleString("pt-BR")}</p>
                  <p className="text-slate-500 text-xs">Curtidas</p>
                </div>
                <div>
                  <p className="text-white font-bold">{post.igMetrics.comments.toLocaleString("pt-BR")}</p>
                  <p className="text-slate-500 text-xs">Comentários</p>
                </div>
                <div>
                  <p className="text-white font-bold">{post.igMetrics.reach.toLocaleString("pt-BR")}</p>
                  <p className="text-slate-500 text-xs">Alcance</p>
                </div>
                <div>
                  <p className="text-white font-bold">{post.igMetrics.impressions.toLocaleString("pt-BR")}</p>
                  <p className="text-slate-500 text-xs">Impressões</p>
                </div>
              </div>
              {post.igMetrics.permalink && (
                <a href={post.igMetrics.permalink} target="_blank" rel="noopener noreferrer"
                  className="mt-2 block text-xs text-blue-400 hover:text-blue-300 transition text-center">
                  Ver no Instagram →
                </a>
              )}
            </div>
